...
# HTML report to https://resolutionscope.com/r/<domain>/<UTC-date>.html
#
...
# bytes. Publishing is one `aws s3 cp` behind the same bucket the site
# deploy uses.
#
# Requires:
...
DOMAIN="${1:?usage: publish-report.sh <domain> [YYYYMMDD]}"
...
aws s3 cp "$OUT" "s3://${RS_S3_BUCKET}/${KEY}" \
