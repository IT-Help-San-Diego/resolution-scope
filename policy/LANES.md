...
2026-08-23T02:51Z | hermes | DANE out-of-zone attribution CONFIRMED REAL-WORLD: Carey's healthcare client had Proofpoint stand up their mail, and Proofpoint failed to update the MTA-STS + MX records — the client's gap was actually the mail operator's. Same shape as dhs.gov (signed gpphosted.com host zone → DnssecRequired never fires → NotConfigured docked, only Proofpoint can fix). Carded: tlsa_zone FIELD (same_zone/different_zone via existing zone_apex_of, no PSL dep) + narrative attribution. Scoped: types(dispositions) → engine(score_dane) → truth_chain → renderer. Open ruling: seal-input vs narrative-only | engine/src/analysis.rs score_dane + types/src/dispositions.rs | @27823b3
